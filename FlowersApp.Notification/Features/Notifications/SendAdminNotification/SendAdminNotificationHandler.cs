...
public class SendAdminNotificationHandler : ICommandHandler<SendAdminNotificationCommand, Guid>
{
    ...
    public async Task<RequestResult<Guid>> Handle(SendAdminNotificationCommand request, CancellationToken cancellationToken)
    {
        _logger.LogInformation("Processing Admin Notification: TargetUserId={TargetUserId}, Type={Type}", request.TargetUserId, request.Type);

        var query = _context.DeviceInstallations.Where(d => d.IsActive);
        if (request.TargetUserId.HasValue)
        {
            query = query.Where(d => d.UserId == request.TargetUserId.Value);
        }

        var activeDevices = await query.ToListAsync(cancellationToken);
        ...
        {
            _logger.LogWarning("No active devices found for Admin notification. Notification created with Completed status.");
            notification.Status = NotificationStatus.Completed;
            await _context.SaveChangesAsync(cancellationToken);
            return RequestResult<Guid>.Succeeded(notification.Id, ResultCode.NoActiveDevicesFound, "No active devices found, notification recorded.");
        }
        ...
        dataPayload["notificationSource"] = NotificationSource.Admin.ToString();
        ...
        return RequestResult<Guid>.Succeeded(notification.Id, ResultCode.NotificationSentSuccessfully, "Admin notification processed.");
    }
}
