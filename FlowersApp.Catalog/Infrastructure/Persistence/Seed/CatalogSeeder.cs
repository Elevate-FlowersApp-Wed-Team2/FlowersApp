...
namespace FlowersApp.Catalog.Infrastructure.Persistence.Seed
{
    public static class CatalogSeeder
    {
        public static async Task SeedAsync(CatalogDbContext context)
        {
            ...
            await SeedProductsAsync(context); // depends on Categories/Occasions existing first
        }
        ...
        //private static async Task SeedSectionsAsync(CatalogDbContext context)
        //{
        //    if (await context.Sections.AnyAsync()) return;

        //    context.Sections.AddRange(
        //        new Section { Type = SectionType.ProductRail, Title = "Featured Products", ArabicTitle = "المنتجات المميزة", Index = 1, IsActive = true, CreatedBy = "seed" },
        //        new Section { Type = SectionType.ProductRail, Title = "New Arrivals", ArabicTitle = "الوافدون الجدد", Index = 2, IsActive = true, CreatedBy = "seed" },
        //        new Section { Type = SectionType.CategoryRail, Title = "Shop by Category", ArabicTitle = "تسوق حسب الفئة", Index = 3, IsActive = true, CreatedBy = "seed" },
        //        new Section { Type = SectionType.OccasionRail, Title = "Special Occasions", ArabicTitle = "المناسبات الخاصة", Index = 4, IsActive = true, CreatedBy = "seed" }
        //    );
        ...
        private static async Task SeedProductsAsync(CatalogDbContext context)
        {
            ...
            var rosesCat = await context.Categories.AsNoTracking().FirstAsync(c => c.Name == "Roses");
            var bouquetsCat = await context.Categories.AsNoTracking().FirstAsync(c => c.Name == "Bouquets");
            var plantsCat = await context.Categories.AsNoTracking().FirstAsync(c => c.Name == "Plants");
            var oldCat = await context.Categories.AsNoTracking().FirstAsync(c => c.Name == "Discontinued Category");

            var wedding = await context.Occasions.AsNoTracking().FirstAsync(o => o.Name == "Wedding");
            var birthday = await context.Occasions.AsNoTracking().FirstAsync(o => o.Name == "Birthday");
            var getWell = await context.Occasions.AsNoTracking().FirstAsync(o => o.Name == "Get Well Soon");
            var graduation = await context.Occasions.AsNoTracking().FirstAsync(o => o.Name == "Graduation");
            var anniversary = await context.Occasions.AsNoTracking().FirstAsync(o => o.Name == "Anniversary");

            // عمل Attach للمناسبات لمنع إعادة إدراجها كـ Entities جديدة
            context.Attach(wedding);
            context.Attach(birthday);
            context.Attach(getWell);
            context.Attach(graduation);
            context.Attach(anniversary);
            ...
            var products = new List<Product>
    {
        ...
        new Product
        {
            ...
            IsActive = false, // Discontinued product للتيست
            CreatedBy = "seed",
            Occasions = new List<Occasion>()
        }
    };
